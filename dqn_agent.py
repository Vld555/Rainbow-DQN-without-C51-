...
class Agent():
    def __init__(self, state_size, action_size, seed, n_step=3):
        ...
        #Q-Network
        self.qnetwork_local = DuelingNoisyQNetwork(state_size, action_size, seed).to(device)
        self.qnetwork_target = DuelingNoisyQNetwork(state_size, action_size, seed).to(device)
        self.optimizer = optim.Adam(self.qnetwork_local.parameters(), lr=LR)
        self.memory = PrioritizingExperienceBuffer(self.state_size, self.action_size, BUFFER_SIZE)
        self.t_step=0
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        if self.t_step == 0:
            if self.memory.real_size > BATCH_SIZE:
                experiences, weights, tree_idxs = self.memory.sample(BATCH_SIZE)
    ...
    def learn(self, experiences, weights, tree_idxs, gamma):
        ...
        # double dqn 
        q_local_best_actions = self.qnetwork_local(next_states).detach().max(1)[1].unsqueeze(1) #выбираем лучшее действие по мнению local
        q_targets_next = self.qnetwork_target(next_states).detach().gather(1, q_local_best_actions)# оцениваем качество выбранных действй
        ...
        self.memory.update_priorities(tree_idxs, td_error.cpu().numpy().squeeze())
        # TODO
